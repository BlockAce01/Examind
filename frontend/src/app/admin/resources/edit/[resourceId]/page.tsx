// src/app/admin/resources/edit/[resourceId]/page.tsx
'use client';
...
import { type Resource } from '@/types/resource'; // Use shared type
...
export default function EditResourcePage() {
    ...
    const { token } = useAuth();
    const resourceId = params.resourceId as string; // Get ID from URL
    ...
    useEffect(() => {
        if (!resourceId || !token) {
            // Handle missing ID or token (maybe redirect or show error)
        ...
        const fetchResource = async () => {
            ...
            try {
                ...
                if (data.status === 'success' && data.data?.resource) {
                    ...
                    setYear(fetchedResource.Year ?? ''); // Use ?? for null/undefined -> empty string
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        // Prepare data payload - **MUST MATCH backend expected keys**
        ...
        try {
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
            const response = await fetch(`${apiUrl}/api/v1/resources/${resourceId}`, { // Use PUT and include ID in URL
            ...
            alert('Resource updated successfully!'); // Replace with toast notification
    ...
    // Display error if fetching failed
     if (error && !title) { // Show fetch error prominently if form couldn't load
         return (
