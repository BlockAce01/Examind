    ...
    const handleDelete = async (resourceId: number, resourceTitle: string) => {
        ...
        if (window.confirm(`Are you sure you want to delete the resource "${resourceTitle}"?`)) {
            try {
                setError(null);
                const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
                const response = await fetch(`${apiUrl}/api/v1/resources/${resourceId}`, { // Use ResourceID
                    method: 'DELETE',
                    headers: {
                                ...
                                resources.map((resource) => (
                                    <tr key={resource.ResourceID} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            <Link href={`/teacher/resources/edit/${resource.ResourceID}`} className="text-indigo-600 hover:text-indigo-900 inline-flex items-center" title="Edit">
                                                <PencilSquareIcon className="w-4 h-4"/>
                                            </Link>
                                            <button
                                                onClick={() => handleDelete(resource.ResourceID, resource.Title || 'this resource')} // Use ResourceID
