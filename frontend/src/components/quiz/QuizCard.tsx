...
import { type QuizListItem } from '@/types/quiz'; // Adjust path if your types folder is elsewhere

interface QuizCardProps {
  quiz: QuizListItem; // Use the shared type
}

const QuizCard: React.FC<QuizCardProps> = ({ quiz }) => {
  // Function to determine difficulty color
  const getDifficultyClass = (level: QuizListItem['DifficultyLevel']) => { // Use type here too
    switch (level) {
    ...
    <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 flex flex-col justify-between h-full">
      <div>
        <div className="flex justify-between items-start mb-2">
            {/* Use correct casing from the type */}
        ...
        <div className="flex items-center text-sm text-gray-500 space-x-4 mb-4">
          <span className="flex items-center">
            ...
            {/* Access questionCount correctly */}
      ...
      <Link
        href={`/quizzes/${quiz.QuizID}/take`} // Use correct casing
