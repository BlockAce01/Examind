...
export interface Resource {
    ...
    Type: 'Past Paper' | 'Model Paper' | 'Notes' | 'Video Lecture' | 'Other'; // Based on proposal examples
    Subject: string; // e.g., "Combined Maths", "Physics", "Chemistry", "Biology"
    FileURL: string; // Link to the actual resource (can be dummy URLs for now)
    Description?: string; // Optional short description
    UploadedDate: string; // Added for potential sorting/display
    year?: number; // Optional: Useful for past/model papers
  }
